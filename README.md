...
Access URL: http://127.0.0.1:8080?token=a3f8b2c1d4e5f6a7
```
...
1. PC と同じ WiFi に接続
2. 起動時に表示されたURL（例: `http://192.168.1.100:8080?token=...`）をスマートフォンのブラウザで開く
...
ssh -L 8080:localhost:8080 user@your-pc-hostname

# または PC 側で以下を実行してから、スマートフォンで http://localhost:8080?token=... にアクセス
ssh -R 8080:localhost:8080 user@your-pc-hostname
```
